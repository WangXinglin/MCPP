...
from .types import expected_node_work_cost, pricing_model_id_for_node
...
def _positive_finite(value: float, fallback: float = 1e-9) -> float:
    try:
        value = float(value)
...
def _rollouts_from_budget(budget_v: float, kappa_v: float) -> int:
    budget_v = _positive_finite(budget_v, fallback=0.0)
    kappa_v = _positive_finite(kappa_v)
    return max(1, int(budget_v / kappa_v))
...
class DispatchOnceStaticPolicy(Policy):
    """
    Shared execution semantics for one-shot static baselines.

    A static baseline may precompute per-node rollout targets and per-node
    budget reservations before simulation starts, but each node is dispatched at
    most once when it first becomes ready. Failed nodes do not receive a second
    batch later in the run.
    """

    dispatch_once_static = True

    def __init__(self):
        self.desired: Dict[int, int] = {}
        self.node_budget_allocations: Dict[int, float] = {}
        self._dispatched_nodes: Set[int] = set()

    def _allocate_undispatched_ready(self, sim: Simulator) -> Dict[int, int]:
        ready = sim.ready_nodes()
        if not ready:
            return {}

        targets = {
            nid: self.desired[nid]
            for nid in ready
            if nid in self.desired and nid not in self._dispatched_nodes
        }
        if targets:
            self._dispatched_nodes.update(targets.keys())
        return targets
...
class SequentialPolicy(Policy):
    ...
    Subtask-level fixed width (English note,English note)
    """
    def __init__(self, rollout_width: int = 1):
        self.rollout_width = max(1, int(rollout_width))
    ...
    def allocate(self, sim: Simulator) -> Dict[int, int]:
        ...
        return {nid: self.rollout_width for nid in ready}


class UniformPolicy(DispatchOnceStaticPolicy):
    """
    ...
    def __init__(self, dag: DAG, budget: float, budget_cost_mode: str = "token_cost_mean"):
        super().__init__()
        ...
        for nid in dag.node_ids:
            ...
            self.node_budget_allocations[nid] = float(max(budget_per_node, 0.0))
            self.desired[nid] = max(1, int(budget_per_node / kappa_v))

    def allocate(self, sim: Simulator) -> Dict[int, int]:
        return self._allocate_undispatched_ready(sim)


class RandomPolicy(DispatchOnceStaticPolicy):
    """
    English note(English note).
    ...
    def __init__(self, dag: DAG, budget: float, seed: int = 0, budget_cost_mode: str = "token_cost_mean"):
        super().__init__()
        ...
        for i, nid in enumerate(dag.node_ids):
            ...
            self.node_budget_allocations[nid] = float(max(budget_v, 0.0))
            self.desired[nid] = _rollouts_from_budget(budget_v, kappa_v)

    def allocate(self, sim: Simulator) -> Dict[int, int]:
        return self._allocate_undispatched_ready(sim)


class LatencyWeightedPolicy(DispatchOnceStaticPolicy):
    """
    English note.
    ...
    English note:
      ...
      4. English note ready English note;English note
    ...
    def __init__(self, dag: DAG, budget: float, budget_cost_mode: str = "token_cost_mean"):
        super().__init__()
        total_lat = sum(
            _positive_finite(dag.nodes[nid].assigned_template.lat_mean)
            for nid in dag.node_ids
        )
        total_lat = _positive_finite(total_lat, fallback=1e-12)

        for nid in dag.node_ids:
            node = dag.nodes[nid]
            tpl = node.assigned_template
            t_v = _positive_finite(tpl.lat_mean)
            kappa_v = expected_node_work_cost(
                tpl,
                node.empirical_samples,
                budget_cost_mode,
...
class SuccessRateWeightedPolicy(DispatchOnceStaticPolicy):
    """
    English note.
    ...
    def __init__(self, dag: DAG, budget: float, budget_cost_mode: str = "token_cost_mean"):
        ...
        for nid in dag.node_ids:
            ...
            try:
                p_v = float(tpl.success_prob)
            except (TypeError, ValueError):
                p_v = 0.0
            if not math.isfinite(p_v):
                p_v = 0.0
            fail_weights[nid] = _positive_finite(1.0 - p_v)  # English note 0

        total_fail = sum(fail_weights.values())
        total_fail = _positive_finite(total_fail, fallback=1e-12)
...
class CriticalPathStaticPolicy(DispatchOnceStaticPolicy):
    """
    English note.
    ...
    def __init__(self, dag: DAG, budget: float, budget_cost_mode: str = "token_cost_mean"):
        super().__init__()
        # English note
        node_cost = {nid: _positive_finite(dag.nodes[nid].assigned_template.lat_mean) for nid in dag.node_ids}
        dist, _ = longest_path_dist_from_nodes(dag, node_cost)
        total_dist = sum(dist[nid] for nid in dag.node_ids)
        total_dist = _positive_finite(total_dist, fallback=1e-12)
...
class LoaderStaticPolicy(Policy):
    ...
    def _run_one_start(
        ...
        # English note restart English note(English note = makespan English note)
            ...
            for path in active_paths:
                ms = sum(
                    expected_latency_surrogate(
                        delta=0.0, t_v=dag.nodes[nid].assigned_template.lat_mean,
                        p_v=dag.nodes[nid].assigned_template.success_prob,
                        r_cur=1.0, r_v=float(best_feasible_plan.get(nid, 1)),
                    )
                    for nid in path
                )
                path_makespans.append(ms)
            obj = max(path_makespans) if path_makespans else float('inf')
            return best_feasible_plan, obj
