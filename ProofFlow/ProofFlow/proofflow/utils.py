...
def count_tokens(messages, model="gpt-3.5-turbo"):
    try:
        ...
        # Fallback for environments that only use remote OpenAI-compatible APIs
        # and do not have tiktoken installed.
        return sum(max(1, len(msg.get("content", "")) // 4) for msg in messages)
...
class LLMManager:
    ...
    def __init__(
        ...
        self.default_max_new_tokens = int(kwargs.pop("default_max_new_tokens", 38912))
        self.default_temperature = float(kwargs.pop("default_temperature", 0.6))
        self.default_top_p = float(kwargs.pop("default_top_p", 0.95))
        self.default_top_k = int(kwargs.pop("default_top_k", 20))
        self.tokenizer = None
        self.model = None
        self.client = None
        self._torch = None
        ...
        if "base_url" in self.model_info:
            if OpenAI is None:
                raise ImportError(
                    "openai is required for OpenAI-compatible API mode. "
                    "Install openai or use --model_backend vllm_local."
    ...
    def call_llm(
        ...
        try:
            if self.client:
                ...
                new_tokens = getattr(usage, "completion_tokens", None)
