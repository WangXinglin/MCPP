...
class BlockedProofItemsError(RuntimeError):
    def __init__(self, blocked_items: List[str], missing_dependencies: Dict[str, List[str]]):
        self.blocked_items = blocked_items
        self.missing_dependencies = missing_dependencies
        super().__init__(
            "No schedulable proof items remain. "
            f"Blocked items: {blocked_items}. Missing dependency context: {missing_dependencies}"
        )
...
class ProofFlow:
    ...
    def __init__(self, 
        ...
        self.save_llm_call_logs = bool(save_llm_call_logs)
        self.llm_call_logs = []  # Stored only when save_llm_call_logs is enabled.
        self.lean_server = lean_server
        self.run_metadata: Dict[str, Any] = {}

    def _llm_logs(self) -> Optional[List[Dict[str, Any]]]:
        return self.llm_call_logs if self.save_llm_call_logs else None
    ...
    def _print_item_progress(self, idx: int, total: int, item_id: str, completed: bool = False) -> None:
        ...
        """
        if completed:
            self._print_status(f"   English note Completed item {item_id}.", style='okgreen')
        else:
            self._print_status(f"English note Formalizing item {idx+1}/{total}: {item_id} ...", style='okblue')

    def _item_is_complete(self, item) -> bool:
        if item.id.startswith("tc_") or item.id.startswith("def_"):
            return getattr(item, "formalization", None) is not None
        return (
            getattr(item, "formalization", None) is not None
            and getattr(item, "solved_lemma", None) is not None
        )

    def _item_has_usable_dependency_context(
        self,
        item,
        ...
        if item.id.startswith("tc_") or item.id.startswith("def_"):
            ...
            return bool(formalization.get("lean_pass") and formalization.get("lean_code"))

        solved_lemma = getattr(item, "solved_lemma", None) or {}
        if solved_lemma.get("lean_verify") and solved_lemma.get("lean_code"):
            return True
    ...
    def _load_or_build_proof_items(
        self,
        nl_proof: str,
        graph_builder_retries: int,
        follow_dag: bool,
        preset_proof_graph: Optional[List[Dict[str, Any]]] = None,
    ) -> None:
        self.nl_proof = nl_proof
        self._print_status("\nBuilding proof graph...", style='bold')

        if preset_proof_graph is not None:
            self.proof_items = validate_proof_graph(deepcopy(preset_proof_graph))
            if not follow_dag:
                condition_on_all_previous_steps(self.proof_items)
            tries = 0
        else:
    ...
    def _supports_threaded_node_execution(self) -> bool:
        managers = [self.formalize_model_manager, self.solver_model_manager]
        return all(getattr(manager, "client", None) is not None for manager in managers)

    def _run_single_item(
        self,
        item,
        formalizer_retries: int,
        prover_retries: int,
        node_rollout_mode: bool,
        rollout_n: int,
        previous_context: bool,
        supply_proof: bool,
        dependency_context_map: Optional[Dict[str, Dict[str, Any]]],
        dependency_context_mode: str,
        ...
        formalization = run_formalizer_prompt(
            ...
            logs=self._llm_logs(),
            max_retries=1 if node_rollout_mode else formalizer_retries,
            rollout_n=rollout_n if node_rollout_mode else 1,
            rollout_parallelism=rollout_parallelism,
            previous_context=previous_context,
            original_proof=self.nl_proof if supply_proof else "",
            dependency_context_map=dependency_context_map,
            dependency_context_mode=dependency_context_mode,
        )
        solved_lemma = run_solver_prompt(
            ...
            logs=self._llm_logs(),
            max_retries=1 if node_rollout_mode else prover_retries,
            rollout_n=rollout_n if node_rollout_mode else 1,
            rollout_parallelism=rollout_parallelism,
        )
        return item.id, formalization, solved_lemma
    ...
    def _process_items_sequential(
        ...
        dependency_context_mode: str,
        rollout_parallelism: int,
        item_completion_callback: Optional[Callable[..., None]],
    ) -> None:
        for idx, item in enumerate(self.proof_items):
            if self._item_is_complete(item):
            ...
            _, formalization, solved_lemma = self._run_single_item(
                item=item,
                formalizer_retries=formalizer_retries,
                prover_retries=prover_retries,
                node_rollout_mode=node_rollout_mode,
                rollout_n=rollout_n,
                previous_context=previous_context,
            ...
            if item_completion_callback:
                item_completion_callback(
                    item=item,
                    item_index=idx,
                    total_items=len(self.proof_items),
    ...
    def _process_items_parallel(
        ...
        if not self._supports_threaded_node_execution():
            raise ValueError(
                "node_parallelism > 1 requires OpenAI-compatible remote backends for the formalizer and solver."
            )

        dependencies = {item.id: set(item.dependencies) for item in self.proof_items}
        item_by_id = {item.id: item for item in self.proof_items}
        index_by_id = {item.id: idx for idx, item in enumerate(self.proof_items)}
        completed: Set[str] = {
            item.id for item in self.proof_items if self._item_is_complete(item)
        }
        in_flight: Dict[Any, Any] = {}

        with ThreadPoolExecutor(max_workers=node_parallelism) as executor:
            while len(completed) < len(self.proof_items):
                precomputed_ids = set(dependency_context_map or {})
                schedule_basis = {
                    item_id
                    for item_id in completed
                    if self._item_has_usable_dependency_context(
                        item_by_id[item_id],
                        dependency_context_mode=dependency_context_mode,
                    )
                } | precomputed_ids
                ...
                for idx, item in enumerate(self.proof_items):
                    if item.id in completed:
                        continue
                    if any(existing.id == item.id for existing in in_flight.values()):
                        continue
                    deps_ready = True
                    if previous_context:
                        if dependency_mode not in {"strict", "best_effort"}:
                            raise ValueError(f"Unsupported dependency_mode: {dependency_mode}")
                        deps_ready = dependencies[item.id] <= schedule_basis
                    if not deps_ready:
                        continue
                    ...
                    if len(in_flight) >= node_parallelism:
                        break
                ...
                if not in_flight:
                    ...
                    missing_dependencies = {
                        item.id: sorted(list(dependencies[item.id] - schedule_basis))
                        for item in self.proof_items
                        if item.id not in completed
                    }
                    raise BlockedProofItemsError(blocked, missing_dependencies)

                done, _ = wait(in_flight.keys(), return_when=FIRST_COMPLETED)
                for future in done:
                    item = in_flight.pop(future)
                    item_id, formalization, solved_lemma = future.result()
                    item.formalization = formalization
                    item.solved_lemma = solved_lemma
                    ...
                    self._print_item_progress(
                        index_by_id[item_id],
                        len(self.proof_items),
                        item_id,
                        completed=True,
                    )
    ...
    def autoformalize_series(self, nl_proof: str,
                             ...
                             node_rollout_mode: bool = False,
                             rollout_n: int = 1,
                             rollout_parallelism: int = 1,
                             node_parallelism: int = 1,
                             dependency_mode: str = "strict",
                             dependency_context_mode: str = "verified_only",
                             ...
                             item_completion_callback: Optional[Callable[..., None]] = None) -> None:
        """
        ...
        Args:
            ...
            node_rollout_mode: Whether to run independent rollout candidates per node and select best successful one
            rollout_n: Number of rollout candidates per node when node_rollout_mode is enabled
            rollout_parallelism: Maximum number of rollout candidates to execute in parallel per node
            node_parallelism: Maximum number of proof DAG nodes to execute concurrently
            dependency_mode: Dependency scheduling mode. Both modes require dependency context; best_effort is currently a compatibility alias.
            dependency_context_mode: Which dependency outputs are eligible as reusable Lean context.
            ...
            preset_proof_graph: Optional validated proof graph payload to reuse instead of rebuilding via the graph model
            dependency_context_map: Optional successful dependency Lean code from prior runs keyed by node id
            resume_existing: Continue from already loaded proof_items instead of rebuilding task state
            item_completion_callback: Called after each node finishes, useful for checkpointing and progress updates
        """
        if resume_existing and self.proof_items is not None:
            self.nl_proof = nl_proof
            self._print_progress_summary(0)
        else:
            self._load_or_build_proof_items(
                nl_proof=nl_proof,
                graph_builder_retries=graph_builder_retries,
                follow_dag=follow_dag,
                preset_proof_graph=preset_proof_graph,
            )
        ...
        if node_parallelism > 1:
            self._process_items_parallel(
                ...
                item_completion_callback=item_completion_callback,
            )
        else:
    ...
    def proof_score(self, verbose: bool = True) -> None:
        ...
        for idx,item in enumerate(self.proof_items):
            if not (hasattr(item, 'score') and item.score):
                if verbose:
                    self._print_status(f"English note Checking score of step {idx+1}/{total}: {item.id} ...", style='okblue')
                semantic_score, feedback = run_node_scorer_prompt(item, model_manager=self.score_model_manager, logs=self._llm_logs())
                item.score = {'semantic_score' : semantic_score, 'semantic_feedback':  feedback}
            if verbose:
                self._print_status(f"   English note Score of step {idx+1}/{total}: {item.id} is {item.score['semantic_score']}", style='okgreen')
