...
def verify_lean_lemma_server(
    ...
    if KiminaClient is None or Snippet is None:
        raise ImportError(
            "kimina_client is required for LeanServer server mode"
        ) from _KIMINA_IMPORT_ERROR
...
def verify_lean_lemma_local(
    ...
    """
    tmp_base = tmp_base or os.environ.get("LEAN_TMP_BASE", "/tmp")
    machine_id = machine_id or os.environ.get(
        "LEAN_MACHINE_ID",
        f"{int(time.time() * 100000)}_{os.getpid()}",
    )
    temp_dir = Path(tmp_base) / "lean_check" / machine_id / f"w{os.getpid()}"
    os.makedirs(temp_dir, exist_ok=True)
    temp_file_path = None
    full_code = f"{LEAN_LIBRARIES}\n\n{lean_string}" if add_imports else lean_string
    env = os.environ.copy()
    lean_home = lean_home or os.environ.get("LEAN_HOME")
    if lean_home:
        env["LEAN_HOME"] = lean_home
        env["PATH"] = f"{lean_home}/bin:" + env.get("PATH", "")
    ...
    try:
        ...
        result = subprocess.run(
            ["lake", "env", "lean", str(temp_file_path), "--json"],
            cwd=project_path,
            capture_output=True,
            text=True,
        ...
        stderr = getattr(e, "stderr", "") or f"Timed out after {timeout}s"
        return (
            False,
            False,
            f"Lean timed out after {timeout}s\nstdout={stdout}\nstderr={stderr}",
        )
    except Exception as e:
        ...
        if temp_file_path is not None:
            stem = temp_file_path.with_suffix("")
            for ext in (".lean", ".olean", ".ilean", ".c", ".trace", ".json", ".out", ".log"):
                try:
                    Path(str(stem) + ext).unlink()
                except FileNotFoundError:
                    pass
                except Exception:
                    pass
...
class LeanServer:
    ...
    def __init__(
        ...
        if project_path:
            ...
            self.tmp_base = tmp_base or os.environ.get("LEAN_TMP_BASE", "/tmp")
            ...
            if KiminaClient is None:
                raise ImportError(
                    "kimina_client is required for LeanServer(api_url=...). "
                    "Install Kimina client dependencies or use LeanServer(project_path=...)"
                ) from _KIMINA_IMPORT_ERROR
            self.mode = "server"
            self.path = api_url
            self.lean_home = None
