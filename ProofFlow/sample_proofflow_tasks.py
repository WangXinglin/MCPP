...
def _task_id(task: dict, fallback_idx: int) -> str:
    if "id" in task and "origin" in task:
        return f"{task['origin']}_{task['id']}"
    if "id" in task:
        return str(task["id"])
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Randomly sample a subset of tasks from ProofFlow benchmark dataset"
    )
    ...
    if args.indices_file:
        ...
        if isinstance(indices_payload, dict):
            if "selected_indices" in indices_payload:
                ...
                raise ValueError("indices_file must contain key 'selected_indices' or 'indices'")
